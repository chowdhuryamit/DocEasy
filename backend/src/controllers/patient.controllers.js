...
const registerPatient = async (req, res) => {
  ...
  try {
    ...
    return res
      ...
      .json({
        success: false,
        msg: "error occured while searching for existing patient",
      });
  }
};
    ...
    try {
       ...
       if (isPasswordValid===false) {
        ...
        try {
            ...
            if (!accessToken) {
                return res.status(200).json({success:false,msg:"access token is not generated"})
            }
            const options = {
               httpOnly: true,
            ...
            .json({success:true,msg:"patient logged in successfully",loggedInPatient})

        } catch (error) {
            return res.status(400).json({success:false,msg:"error occured while generating access token"})
        }
    } catch (error) {
        return res.status(400).json({success: false,msg: "error occured while validaing patient password"})
    }
  } catch (error) {
...
const logoutPatient = async (req,res) => {
    ...
    if(req.patient===null){
       return res.status(200).json({success:false,msg:'patient is not logged in'})
    }
...
const getPatient = async (req, res) => {
    ...
    try {
        const patientData=await Patient.findById(req.patient._id).select("-password")

        if(!patientData){
            return res.status(200).json({success:false,msg:'patient not found'})
        }

        return res.status(200).json({success:true,msg:'patient found',patientData});
